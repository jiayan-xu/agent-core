#!/usr/bin/env python3
"""受控写总闸回归（白名单 / 异常同步 / 取样台账 / 短确认假成功）。

覆盖：
  1) 离线：cargo test（DSML / 预路由 / v1_compat / controlled_write / 注册表地板）
...
运行：
  python scripts/e2e_controlled_write.py
  python scripts/e2e_controlled_write.py --live
  python scripts/e2e_controlled_write.py --full
  # 兼容旧入口：
...
def approve_item(item: dict) -> None:
    ...
    r = http_json(
        ...
        admin_headers(),
        {"approved": True, "reason": "e2e_controlled_write --full", "operation_hash": oh},
    )
...
def run_live() -> None:
    ...
    sid = "e2e/controlled_write/" + time.strftime("%Y%m%d_%H%M%S")
...
def run_full() -> None:
    ...
    sid = "e2e/controlled_write/full/" + time.strftime("%Y%m%d_%H%M%S")
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="受控写总闸回归")
