...
### P2-2 审计增强（已完成，commit 待 push）

- 统一事件（9 类，`audit.rs` `AuditEventType`）：`AuthFail` / `BoundaryDeny` / `ApprovalCreated|Approved|Rejected` / `McpRetry` / `CheckpointResume` / `HarnessHit` / `ToolInvocation` / `IdentityChange`；serde `snake_case` 稳定字符串。
- 每条事件带 `trace_id`（纳秒时间戳 + `AtomicU64` 自增序号，免 uuid 依赖）+ 可选 `session_id` + 已脱敏 `detail`；同一次请求链路（chat→llm_loop→call_tool_routed→边界/MCP）共享 `trace_id`。
- 双写：异步写 Memoria `memory_observe`（耐久，失败忽略）+ 本地有界环形缓冲（2000 条，`Mutex<VecDeque>`），`/api/admin/audit`（GET，需鉴权）即时返回，支持 `trace_id` / `event` / `limit` 过滤。
- 埋点：`chat()` 生成 `trace_id`；`llm_loop` 边界拒绝改 `boundary_deny(trace_id, session_id)`、审批创建 `approval_event("created", ...)`、日 token 预算检查；`call_tool_routed` 实际执行 `mcp_retry`；`restore_checkpoint` `CheckpointResume`；`try_harness_match` 命中 `HarnessHit`；鉴权中间件失败 `auth_fail`。
- 敏感字段脱敏（`redact`：admin_key/api_key/token/password/secret/authorization 等值替换为 `***`），单测 `redact_masks_api_key_value` 覆盖。
- 联调验证：无身份访问受保护端点返回 401 并写入 `auth_fail`，`/api/admin/audit` 可查回（含 `trace_id`/`ts`/`event_type`/`detail`）。

### P2-3 Harness 蒸馏质量（已完成，commit 待 push）

- 蒸馏触发门槛：agent 侧 `distill_from_logs(&logs, 3)`（N=3 次成功组合路由佐证），避免偶发成功被过早蒸馏为模板。
- 危险模板永不自动激活：`boundary::is_dangerous_tool`（内置危险清单 + `delete_`/`batch_delete`/`shutdown_` 高危前缀）命中即把蒸馏出的模板置 `is_active=0`（待审批）；安全模板（只读/写）仍自动激活。
- 审批激活路径：`HarnessStore::activate(id)`（逆向 `deactivate`）+ 新增 `POST /api/admin/harness/activate`（需鉴权）供人工 / admin 批准上线。
- 可观测：Harness 命中写入 `match_score` + `harness` 名到 tracing span；`call_tool_routed` 的 harness 步骤与 `llm_loop` 的工具调用均把 `boundary` 的 `allowed` / `level` / `reason` 写入 span（P2-3）。
- 单测：`test_distill_dangerous_template_not_auto_activated` / `test_distill_safe_template_auto_activated` / `test_activate_toggles_is_active` 覆盖；lib 全量 106 pass。
...
### P2-5 桌面壳边界清晰（已完成，commit 待 push）

- 新增 `docs/SHELL_ENGINE_BOUNDARY.md`：固化「壳只做壳、一切经引擎」铁律 + 三张 Mermaid 序列图
  （正常对话 / 注册登录代理 / 反模式禁止），明确壳侧**不得**持有 `memoria_admin_key`、**不得**直连 `:9003`、
  禁止在壳侧分叉实现一套直连 Memoria 的鉴权/记忆逻辑（绕过统一鉴权、边界闸门与审计）。
- README 拓扑段补链接指向该文档。
- 配套 ADR：`docs/decisions/ADR-003-unified-auth-localhost.md`（统一鉴权 / 本机默认）。
...
| P2-1 | 配额与成本 | W3 | 已完成 |
| P2-2 | 审计增强 | W3 | 已完成 |
| P2-3 | Harness 质量 | W3 | 已完成 |
| P2-4 | DX / 文档 / 示例 | W3 | 已完成 |
| P2-5 | 壳与引擎边界文档 | W3 | 已完成 |
| P2-6 | 配置与密钥收干净 | W3 | 已完成 |
...
- **ADR 索引**（`docs/decisions/`）：
  - `ADR-002` Composer 组合技能路由（既有）
  - `ADR-003` 统一鉴权 + 本机默认（P0-1）
  - `ADR-004` Checkpoint 控制面落盘（P1-1）
  - `ADR-005` Tracing 可观测底盘（P0-3）
  - `ADR-006` 审计增强 统一事件模型 + trace_id（P2-2）
  - `ADR-007` Harness 蒸馏质量 危险模板不自动激活（P2-3）
  - `ADR-008` 危险工具硬闸门（P0-2）
  - `ADR-009` 边界策略收紧（P0-4）
  - `ADR-010` Composer 计划 HITL 产品化（P1-2）
  - `ADR-011` 场景回归评测集（P1-3）
  - `ADR-012` LLM / 工具契约硬化（P1-4）
  - `ADR-013` 降级收缩显式化（P1-5）
  - `ADR-014` 命名空间级配额与成本（P2-1）
