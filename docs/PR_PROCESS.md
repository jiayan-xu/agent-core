...
> 适用仓库：GitHub `jiayan-xu/agent-core`（本地 canonical 见 AGENTS.md）
> 更新日期：2026-08-13
...
1. **禁止直接 push 到 `master`**（pre-push hook 已拦截，`--no-verify` 绕过被记录为违规）。
2. **所有改动必须走 PR**：feature 分支 → push → 开 PR → CI 通过 → 合入 master。
3. **PR 合入时 required checks 必须全绿**（ocr-review + gitleaks），否则合并按钮灰掉。
4. **禁止 force push 到 master / 删除分支**（分支保护已开，hook 也拦）。
...
```bash
git fetch origin
git checkout -b feat/<简述> origin/master   # 如 feat/write-intent-cap
```
...
预估：ocr-review + gitleaks 约 1–3 分钟。watch 结束前不要结束对话。
...
| 推送到 | 行为 |
|---|---|
| `master`（默认分支） | **BLOCKED**：要求走 PR（本文件第 3 节） |
