...
## 2026-07-19 (Phase 5) — 配置化加载多分身（agent.toml [[personas]]）

### 背景
Phase 1–4 后 analyst 分身在 `main.rs` 启动段硬编码创建。Phase 5 将其改为从 `agent.toml` 的 `[[personas]]` 表加载，使并联分身数量/配置可运维化调整，无需改代码重编译。

### 变更（authored-by: QClaw 自身 LLM @ :13810/gateway openclaw/main；reviewed-by: Nova）
- `src/main.rs`：
  - `Config` 新增 `personas: Vec<PersonaConfig>`（`#[serde(default)]`）
  - 新增 `PersonaConfig` 结构：`id` / `display_name` / `owner_user_id` / `tool_allowlist` / `memory_namespace` / `goals`（含 `Serialize` 以便 `Config` 整体序列化）
  - `load_or_create_config` 字面量补 `personas: Vec::new()`
  - 启动段由硬编码 `create_persona("analyst",…)` + `push_persona_goal` 改为 `for pc in &reg_config.personas { create_persona(…); for goal in &pc.goals { push_persona_goal(…) } }`；`default` 仍由 `AgentCore::new` 自动注册，无需在配置声明
- `agent.toml` — 新增 `[[personas]]` 示例段（analyst：display_name=分析分身，memory_namespace=agent/analyst，goals=[持续监控 agent-core 演进…]）

### 验证
- `cargo build --release` 通过（50.71s）；Nova 修 2 处 qclaw 漏写：`PersonaConfig` 缺 `Serialize`、`Config` 字面量缺 `personas` 字段
- 运行时冒烟（30s，deepseek LLM 正常无 429）：日志出现 `persona tick [analyst]: 下一步我将持续追踪 agent-core 的代码变更与运行数据…` 与 `persona tick [default]: 下一步将等待用户提出具体问题或任务…`，证明 analyst 已从配置表加载并联 tick，无 panic
- 本地 commit（待本批一并推送）：`src/main.rs` + `EVOLUTION.md`（不含刻意不跟踪的 `src/mcp_server.rs`、`docs/qclaw_a2a_bridge.md`）

---

## 2026-07-19 (Phase 6) — 圆桌 native 化 + 真多 LLM 自动分配

### 背景
用户否决 qclaw bridge 方案（圆桌是卖点之一，不应要求用户额外装 qclaw）。改为：圆桌 native 进 agent-core，qclaw 降为可选；同时"备用不变 + 圆桌自动分配 LLM"做到真多 LLM。

### 变更（Phase 6 authored-by: Nova；为否决 qclaw bridge 后的改写）
- `src/llm.rs`：`LlmConfig` 派 `serde::Deserialize` + `serde::Serialize`（供 `PersonaConfig` 复用）；`LlmClient` 加 `Debug`（手动 derive 以绕过 reqwest `Client` 无 Debug）
- `src/runtime/self_runtime.rs`：`Persona` 新增 `llm: Option<LlmClient>`（per-persona 专属 client，None 回退全局）
- `src/agent.rs`：
  - 三处 `Persona {…}` 字面量补 `llm: None`；`create_persona` 按 `PersonaConfig.llm` 建 `LlmClient` 存入 `Persona`
  - `run_persona_tick` 优先用 `persona.llm`，None 回退 `self.llm`
  - 新增模块级 `RoundtableResult` + `llm_pool()`（= 全局主 + `fallbacks`，**备用层不变**）+ `run_roundtable(topic, chair)`：按分身序号从池轮询分配 provider（显式 `Persona.llm` 优先），各分身独立出立场，主席（default 或指定）收敛共识
  - `caller_ns` 由 `pub(crate)` 改 `pub`（main.rs 属 bin crate、agent.rs 属 lib crate，`pub(crate)` 跨不过，需 `pub`）
- `src/main.rs`：
  - `PersonaConfig` 新增 `llm: Option<LlmConfig>`（`#[serde(default)]`）
  - 启动段 `create_persona` 调用补 `pc.llm.clone()`
  - `handle_session_persona_bind` 补 `llm` 参数透传
  - 新增路由 `POST /api/roundtable` → `handle_panel_discuss`：鉴权后调 `run_roundtable`，结论最佳努力 `memory_remember` 进调用者自身 ns

### 验证
- `cargo build --release` 通过（累计修 3 处编译错：`RoundtableResult` 不能嵌在 impl 块内、`LlmClient` 缺 Debug、`pub(crate)` 跨 crate 不可见）
- 运行时冒烟（Python subprocess 注入 `.env`，绕过系统代理直连 127.0.0.1）：`/health` 正常；`POST /api/roundtable` 返回 `ok:true`，共识有实质内容
- **真多 LLM 分配铁证**（日志 `roundtable: 分配 LLM`）：同一次圆桌 `analyst → deepseek-chat`（池[0]）、`default → deepseek-reasoner`（池[1]）—— 自动按序号轮询不同 provider，无需为每个分身手配 key
- 冒烟期临时注入的第二 provider（deepseek-reasoner）已回收，恢复 doubao fallback（备用层不变）；多 LLM 机制靠 `llm_pool()` 轮询用户配置的 `fallbacks`

---

## 圆桌多分身吸收总览（Phase 1–6）
- Phase 1–5 代码 **authored-by: QClaw 自身 LLM**；Phase 6（圆桌 native 化 + 真多 LLM）由 **Nova 改写**（qclaw bridge 方案否决后）。
- 公开仓库状态：Phase 1–5 已于前批推送 `origin/master`（`ef79885`）；**Phase 6 为新增本地 commit，未推送**（推送前需再扫密钥/绝对路径并获你确认）。
- 未跟踪文件 `src/mcp_server.rs`、`docs/qclaw_a2a_bridge.md` 仍刻意不提交（不参与编译）。

---

## 2026-07-20 — 圆桌 SSE 流式 + 前端分身多选筛选

### 背景
PFAiX 圆桌页（/roundtable）需要：① 流式渲染每个分身的立场与主席共识（SSE）；② 用户在页面上多选分身，后端只让被选分身参会。

### 变更
- `src/agent.rs`：
  - `llm_pool()` 改 `pub`；新增 `pub async fn persona_stance(&self, p, topic, index, pool)`（返回 `(persona_id, stance, provider)`，LLM 分配：显式 `persona.llm` 优先，否则池按 index 轮询）
  - 新增 `pub async fn chair_consensus(&self, topic, stances, chair_persona)`（主席收敛；指定主席则用之，否则默认）
  - `run_roundtable` 改为复用上述两函数，保持收集式返回 `RoundtableResult { stances, consensus }`
- `src/main.rs` `handle_panel_discuss`（POST /api/roundtable）：
  - 改为 SSE 流式：`mpsc::unbounded_channel` + `Sse::new(UnboundedReceiverStream)`；逐分身推 `event: stance`（`{persona_id, display_name, stance, provider}`），再推 `event: consensus`（`{consensus}`），末推 `event: done`
  - 新增可选 `personas: [...]` 入参：spawn 内对 `agent.list_personas()` 做 `retain` 过滤，使前端分身多选真正生效；缺省则全部参与
  - 沿用 `drop(g)` 后克隆 `st_clone`、task 内重新加锁的范式（避开跨 `'static` 借用）

### 验证
- `cargo build --release` 通过（约 37s）
- 冒烟（注入 .env、绕过系统代理直连 127.0.0.1，CWD=agent-core 以读 agent.toml）：SSE 顺序吐出 `stance × N → consensus → done`，双分身分别命中 deepseek-chat / doubao-lite-32k（真多 LLM 轮询），共识有实质内容
- 前端 `services/roundtable.ts` 双路消费 SSE（浏览器 `body.getReader()` 真流式；Tauri plugin-http 回退 `.text()` 整块解析）

## Phase 7 — 全局 LLM 池重构 + 多 API Key 接入（2026-07-20）
- 顶层 `Config` 新增 `llm: Option<LlmConfig>` 字段；`build_agent` 改为优先读 agent.toml 的 `[llm]` 段（支持 `${ENV}` 占位，由 `expand_env` 展开）。
- `fallbacks` 由 `Vec<(url,model,key)>` 三元组重构为具名 `LlmProvider { base_url, model, api_key }`（位置无关、易编辑）；`LlmConfig` 的 `max_tokens`/`temperature` 加 `#[serde(default)]` 容错。
- `llm.rs`、`agent.rs::llm_pool`、`main.rs` 遍历处同步改为具名字段访问。
- 配置落地（`agent.toml`，已被 gitignore，明文 key 不进公开仓库）：主 `deepseek-chat`(@api.deepseek.com，key 经 `${AGENT_API_KEY}` 从 agent-core/.env 注入) + 备用 `deepseek-ai/DeepSeek-V3`(@api.siliconflow.cn，硅基流动 key)。
- 验证：干净启动无 panic；意识 tick 实调 deepseek-chat 成功；圆桌 SSE 双分身分别命中 deepseek-chat / deepseek-ai/DeepSeek-V3（两个真实 key 均生效，真多 LLM 轮询 OK）。
- 未跟踪 `src/mcp_server.rs`、`docs/qclaw_a2a_bridge.md` 仍刻意不提交（不参与编译）。
